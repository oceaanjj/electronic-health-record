(function () {
    function initializeIntakeOutputDataLoader() {
        const patientSelectForm = document.getElementById(
            "patient-select-form"
        );
        if (!patientSelectForm) {
            console.warn(
                "Intake/Output Data Loader: #patient-select-form not found."
            );
            return;
        }
        ...
        if (!dayNoSelector || !patientIdHiddenInput || !ioForm) {
            console.error(
                "Intake/Output Data Loader: Missing one or more required elements (day, patient_id hidden input, or io-form)."
            );
            return;
        }
        ...
        const analyzeUrl = dropdownContainer
        ...
        if (!analyzeUrl || !csrfToken) {
            console.error(
                "Intake/Output Data Loader: Form missing action URL or CSRF token not found."
            );
            return;
        }
        ...
        const fetchIntakeOutputData = async () => {
            ...
            // The dayNo from the selector is just a starting point, controller will recalculate
            ...
            console.log(
                `Fetching data for Patient ID: ${patientId}, (requesting day: ${dayNo})`
            );
            ...
            try {
                const response = await fetch(analyzeUrl, {
                    ...
                    body: JSON.stringify({
                        ...
                        day_no: dayNo, // Send this, but controller will override
                    }),
                });
                ...
                // Update the (disabled) Day No selector to the correct calculated day
                if (dayNoSelector) {
                    dayNoSelector.value = data.currentDayNo;
                }
                // --- END: MODIFIED SECTION ---

                // Update form fields
                const oralIntakeInput = ioForm.querySelector(
                    '[name="oral_intake"]'
                );
                const ivFluidsInput = ioForm.querySelector(
                    '[name="iv_fluids_volume"]'
                );
                const urineOutputInput = ioForm.querySelector(
                    '[name="urine_output"]'
                );

                if (oralIntakeInput)
                    oralIntakeInput.value = data.ioData?.oral_intake ?? "";
                if (ivFluidsInput)
                    ivFluidsInput.value = data.ioData?.iv_fluids_volume ?? "";
                if (urineOutputInput)
                    urineOutputInput.value = data.ioData?.urine_output ?? "";

                // Update hidden fields in the io-form
                const hiddenDayNoInput = ioForm.querySelector(
                    'input[name="day_no"]'
                );
                if (hiddenDayNoInput)
                    hiddenDayNoInput.value = data.currentDayNo;

                // Dispatch a custom event to notify other scripts (e.g., CDSS) that data has been loaded
                const event = new CustomEvent("io:data-loaded", {
                    detail: { ioData: data.ioData },
                });
        ...
        //  trigger on patient selection change (if patient_id_hidden changes)
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (
                    ...
                    if (
                        patientIdHiddenInput.value &&
                        mutation.oldValue !== patientIdHiddenInput.value
                    ) {
        ...
        observer.observe(patientIdHiddenInput, {
            attributes: true,
    ...
    document.addEventListener(
        "DOMContentLoaded",
        initializeIntakeOutputDataLoader
    );
})();
