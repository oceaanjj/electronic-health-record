...
<body>
    ...
    {{-- FORM for patient selection (submits with GET) --}}
    <form action="{{ route('medical-history') }}" method="GET">
        <div class="container">
            <div class="header">
                <label for="patient_id">PATIENT NAME :</label>
                {{-- Patient Name DROPDOWN. 'onchange' submits the form to reload the page with the selected patient's data --}}
    ...
    <form action="{{ route('medical.store') }}" method="POST">
        ...
        <table>
            ...
            <tr>
                <td><input type="text" name="present_condition_name" value="{{ $presentIllness->condition_name ?? '' }}"></td>
            ...
            <tr>
                <td><input type="text" name="past_condition_name" value="{{ $pastMedicalSurgical->condition_name ?? '' }}"></td>
            ...
            <tr>
                <td><input type="text" name="allergy_condition_name" value="{{ $allergy->condition_name ?? '' }}"></td>
            ...
            <tr>
                <th>GROSS MOTOR</th>
                <td colspan="6"><textarea name="gross_motor">{{ $developmentalHistory->gross_motor ?? '' }}</textarea></td>
            </tr>
            <tr>
                <th>FINE MOTOR</th>
                <td colspan="6"><textarea name="fine_motor">{{ $developmentalHistory->fine_motor ?? '' }}</textarea></td>
            </tr>
            <tr>
                <th>LANGUAGE</th>
                <td colspan="6"><textarea name="language">{{ $developmentalHistory->language ?? '' }}</textarea></td>
            </tr>
            <tr>
                <th>COGNITIVE</th>
                <td colspan="6"><textarea name="cognitive">{{ $developmentalHistory->cognitive ?? '' }}</textarea></td>
            </tr>
            <tr>
                <th>SOCIAL</th>
