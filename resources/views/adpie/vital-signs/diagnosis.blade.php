...
@section('content')
    ...
    <form
        action="{{ route('nursing-diagnosis.storeDiagnosis', ['component' => $component, 'id' => $patient->patient_id]) }}"
        method="POST"
        ...
        <fieldset>
            <div class="mx-auto mt-6 flex w-[70%] items-start justify-center gap-0">
                <div class="w-[68%] overflow-hidden rounded-[15px]">
                    <div class="main-header rounded-t-lg py-2 text-center font-bold text-white">
...
{{ old('diagnosis', $diagnosis->diagnosis ?? '') }}</textarea
                ...
                <div class="bg-beige ml-4 w-[25%] overflow-hidden rounded-[15px]">
                    <div class="main-header rounded-t-lg py-2 text-center font-bold text-white">RECOMMENDATIONS</div>
                    {{-- NEW: Pre-load alert from session ★ --}}
                    @if ($component === 'vital-signs')
                        @php
                            $alert = session('vital-signs-alerts')['diagnosis'] ?? null;
            ...
            <div class="bg-beige mx-auto mt-6 flex w-[70%] items-center justify-between">
                ...
                <div class="bg-beige flex flex-row items-center justify-end space-x-2">
