            ...
            <form id="vitals-form" class="cdss-form" method="POST" action="{{ route('vital-signs.store') }}"
                    ...
                    <div class="relative w-full md:w-[30%]">
                        <div id="chart-viewport" class="relative overflow-hidden rounded-[25px]">
                            <div id="chart-track" class="transition-transform duration-700 ease-out">
                                <div class="h-[220px] rounded-[24px] border border-white bg-gradient-to-br from-white via-[#edecec] to-[#f1f5f9] p-4 pb-12 shadow-sm">
                                    <h2 class="mb-2 text-center text-sm font-bold text-[#334155] uppercase opacity-80">TEMPERATURE CHART</h2>
                                    <canvas id="tempChart"></canvas>
                                </div>
                                <div class="h-[220px] rounded-[24px] border border-white bg-gradient-to-br from-white via-[#edecec] to-[#f1f5f9] p-4 pb-12 shadow-sm">
                                    <h2 class="mb-2 text-center text-sm font-bold text-[#334155] uppercase opacity-80">HEART RATE CHART</h2>
                                    <canvas id="hrChart"></canvas>
                                </div>
                                <div class="h-[220px] rounded-[24px] border border-white bg-gradient-to-br from-white via-[#edecec] to-[#f1f5f9] p-4 pb-12 shadow-sm">
                                    <h2 class="mb-2 text-center text-sm font-bold text-[#334155] uppercase opacity-80">RESPIRATORY RATE CHART</h2>
                                    <canvas id="rrChart"></canvas>
                                </div>
                                <div class="h-[220px] rounded-[24px] border border-white bg-gradient-to-br from-white via-[#edecec] to-[#f1f5f9] p-4 pb-12 shadow-sm">
                                    <h2 class="mb-2 text-center text-sm font-bold text-[#334155] uppercase opacity-80">BLOOD PRESSURE CHART</h2>
                                    <canvas id="bpChart"></canvas>
                                </div>
                                <div class="h-[220px] rounded-[24px] border border-white bg-gradient-to-br from-white via-[#edecec] to-[#f1f5f9] p-4 pb-12 shadow-sm">
                                    <h2 class="mb-2 text-center text-sm font-bold text-[#334155] uppercase opacity-80">SpO₂ CHART</h2>
                                    <canvas id="spo2Chart"></canvas>
                                </div>
                            </div>
                        </div>

                        <button id="chart-up" type="button" class="btn-hidden absolute -top-8 left-1/2 z-30 flex h-10 w-10 -translate-x-1/2 items-center justify-center rounded-full border border-[#e2e8f0] bg-white text-[#64748b] shadow-sm">
                            <span class="material-symbols-outlined text-[32px]">arrow_drop_up</span>
                        </button>
                        <button id="chart-down" type="button" class="absolute -bottom-8 left-1/2 z-30 flex h-10 w-10 -translate-x-1/2 items-center justify-center rounded-full border border-[#e2e8f0] bg-white text-[#64748b] shadow-sm">
                        ...
                        <div class="space-y-4 md:hidden">
                            @foreach ($times as $time)
                                ...
                                <div class="relative mb-6 flex w-full flex-col overflow-hidden rounded-[15px] border border-[#c18b04] bg-beige">
                                    ...
                                    <div class="absolute right-4 top-2.5 z-10 flex items-center justify-center mobile-alert-clone" data-alert-for-time="{{ $time }}">
                                        <div class="alert-icon-btn is-empty">
                                            <span class="material-symbols-outlined">notifications</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-2">
                                        @foreach (['temperature' => 'temperature', 'hr' => 'bpm', 'rr' => 'bpm', 'bp' => 'mmHg', 'spo2' => '%'] as $field => $unit)
                                            <div class="mb-2 w-full">
                                                ...
                                                {{-- REMOVED NAME ATTRIBUTE FROM MOBILE TO PREVENT OVERWRITING DESKTOP DATA --}}
                                                <input type="text"
                                                    placeholder="{{ $unit }}"
                ...
                <div class="mx-auto mt-5 mb-20 flex w-full justify-center space-x-4 md:w-[95%] md:justify-end">
                    @if (isset($vitalsData) && $vitalsData->count() > 0)
                        <button type="submit" formaction="{{ route('vital-signs.cdss') }}" class="button-default text-center cdss-btn">CDSS</button>
                    @endif
        ...
        <div id="chart-modal">
            <div class="modal-container">
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <button type="button" onclick="closeChartModal()" class="cursor-pointer rounded-full p-2 hover:bg-gray-100">
