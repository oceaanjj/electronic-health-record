...
class ActOfDailyLivingCdssService
{
    ...
    /**
     * Loads and correctly merges all YAML rule files from the act_of_daily_living directory.
     */
            ...
            if (in_array($file->getExtension(), ['yaml', 'yml'])) {
                try {
                    ...
                    if (is_array($parsedYaml)) {
                        ...
                        // For example, if two files define rules for 'mobility_assessment',
    ...
    public function analyzeFindings($findingsData)
    {
        ...
        foreach ($keysToAnalyze as $key) {
            ...
            if ($alert) {
                // The session key is the same as the field name for ADL
                $alerts[$key] = $alert;
            }
        }
        return $alerts;
