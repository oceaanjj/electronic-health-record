...
// =======================================================
// CDSS AVAILABILITY BANNER
// Shows a dismissible banner near the patient header when CDSS is available
// =======================================================
...
function patientHasCdss() {
    // Presence of a `.cdss-btn` in the current form indicates server-side availability
    return !!document.querySelector(".cdss-btn");
}

function cdssOpenedKey(patientId) {
    return `cdss_opened_${patientId}`;
}
...
function cdssDismissedKey(patientId) {
    return `cdss_banner_dismissed_${patientId}`;
}

function markCdssOpened(patientId) {
    if (!patientId) return;
    try {
        sessionStorage.setItem(cdssOpenedKey(patientId), "1");
    } catch (e) {
        console.warn("Could not set sessionStorage for cdss opened", e);
    }
}

function hasOpenedCdss(patientId) {
    if (!patientId) return false;
    try {
        return sessionStorage.getItem(cdssOpenedKey(patientId)) === "1";
    } catch (e) {
        return false;
    }
}

function bannerDismissed(patientId) {
    if (!patientId) return false;
    try {
        return localStorage.getItem(cdssDismissedKey(patientId)) === "1";
    } catch (e) {
        return false;
    }
}

function dismissBannerForPatient(patientId) {
    if (!patientId) return;
    try {
        localStorage.setItem(cdssDismissedKey(patientId), "1");
    } catch (e) {
        console.warn("Could not set localStorage for banner dismissed", e);
    }
    removeCdssBanner();
}
...
function removeCdssBanner() {
    const existing = document.querySelector(".cdss-availability-banner");
    if (existing) existing.remove();
}

function renderCdssBanner() {
    const patientId = getCurrentPatientId();
    if (!document.querySelector(".header")) return;

    // Remove previous banner first
    removeCdssBanner();

    const hasCdss = patientHasCdss();
    const opened = hasOpenedCdss(patientId);
    const dismissed = bannerDismissed(patientId);

    // If banner was dismissed by user, do not show again
    if (dismissed) return;

    const banner = document.createElement("div");
    banner.className = "cdss-availability-banner w-full rounded px-4 py-3 my-2 text-sm flex items-center justify-between";

    // Basic styling (keeps consistent with existing styles)
    banner.style.background = hasCdss && !opened ? "#FFEE99" : "#F3F4F6"; // yellow when available, gray otherwise
    banner.style.border = "1px solid rgba(0,0,0,0.06)";
    banner.style.color = "#083344";

    const message = document.createElement("div");
    if (hasCdss && !opened) {
        message.textContent = "Clinical Decision Support System is now available.";
    } else if (hasCdss && opened) {
        // If already opened, do not show anything
        return;
    } else {
        message.textContent = "Clinical Decision Support System not yet available for this patient.";
    }
    ...
    // Optional 'Open CDSS' quick link when available
    if (hasCdss && !opened) {
        const openBtn = document.querySelector(".cdss-btn");
        if (openBtn) {
            const link = document.createElement("a");
            link.href = openBtn.href || "#";
            link.className = "button-default";
            link.textContent = "Open CDSS";
            link.addEventListener("click", () => {
                markCdssOpened(patientId);
                // Banner will be removed after open
                removeCdssBanner();
            });
    ...
    closeBtn.className = "close-cdss-banner";
    closeBtn.textContent = "×";
    closeBtn.style.background = "transparent";
    closeBtn.style.border = "none";
    closeBtn.style.fontSize = "20px";
    closeBtn.style.cursor = "pointer";
    closeBtn.addEventListener("click", () => dismissBannerForPatient(patientId));
    ...
    // Insert banner below the header (patient selection area)
    const header = document.querySelector(".header");
    header.parentNode.insertBefore(banner, header.nextSibling);
}

// Attach listener to CDSS button so we can mark it opened when clicked
function attachCdssButtonOpenListener() {
    const cdssBtn = document.querySelector(".cdss-btn");
    if (!cdssBtn) return;
    const patientId = getCurrentPatientId();
    cdssBtn.addEventListener("click", () => markCdssOpened(patientId));
}

// Initialize banner on DOM load and when forms reload
document.addEventListener("DOMContentLoaded", () => {
    try {
        renderCdssBanner();
        attachCdssButtonOpenListener();
    } catch (e) {
        console.warn("CDSS banner init failed", e);
    }
});

document.addEventListener("cdss:form-reloaded", () => {
    try {
        renderCdssBanner();
        attachCdssButtonOpenListener();
    } catch (e) {
        console.warn("CDSS banner reload failed", e);
    }
});
