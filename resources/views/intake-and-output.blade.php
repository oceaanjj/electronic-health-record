...
@section('content')
    ...
    <div id="form-content-container" class="mx-auto max-w-full">
                ...
                <div id="cdss-alert-wrapper" class="mx-auto w-[85%] overflow-hidden transition-all duration-500">
                    <div id="cdss-alert-content"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm font-semibold text-[#dcb44e]">Clinical Decision Support System is now
                                available.</span>
                        </div>
                        <button type="button" onclick="closeCdssAlert()"
                            ...
                            <span
                                class="material-symbols-outlined text-[20px] transition-transform duration-300 group-hover:rotate-90">close</span>
                        </button>
                    </div>
                </div>
            @endif
        ...
        <div class="mx-auto mt-1 w-[90%] max-w-full">
            <div class="mx-auto w-full pt-10">
                <div
                    class="mobile-dropdown-container mx-auto mb-5 flex w-[85%] flex-wrap items-center gap-x-10 gap-y-4 md:ml-24">
        ...
        <fieldset @if(!$selectedPatient) disabled @endif>
            <form id="io-form" class="cdss-form" method="POST" action="{{ route('io.store') }}"
                ...
                <div
                    class="mt-5 ml-5 flex w-[90%] max-w-screen-2xl flex-col items-center gap-5 md:mt-10 md:ml-40 md:flex-row md:items-center md:justify-start md:gap-0">

                    <div
                        class="relative w-[85%] mobile-card-wrapper bg-white md:bg-transparent md:border-none md:rounded-none flex flex-col md:flex-row md:items-center">
                        ...
                        <div class="w-full overflow-hidden rounded-[15px] md:rounded-[15px] md:border-none">
                            <table class="w-full table-fixed border-collapse border-spacing-y-0 responsive-table">
                                ...
                                <tr class="bg-beige text-brown responsive-table-data-row">
                                    <td class="text-center align-middle responsive-table-data"
                                        data-label="ORAL INTAKE (mL)">
                                        <input type="text" name="oral_intake" placeholder="Enter Oral Intake"
                                            value="{{ old('oral_intake', $ioData->oral_intake ?? '') }}"
                                            oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1')"
                                            class="bg-beige cdss-input vital-input h-[100px] w-full px-3 text-center focus:outline-none"
                                    ...
                                    <td class="text-center align-middle responsive-table-data" data-label="IV FLUIDS (mL)">
                                        <input type="text" name="iv_fluids_volume" placeholder="Enter IV Fluids"
                                            ...
                                            oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1')"
                                            class="bg-beige text-brown cdss-input vital-input h-[100px] w-full px-3 text-center focus:outline-none"
                                            data-field-name="iv_fluids_volume" data-time="io" />
                                    </td>

                                    <td class="text-center align-middle responsive-table-data"
                                        data-label="URINE OUTPUT (mL)">
                ...
                <div
                    class="mx-auto mt-5 mb-30 flex w-[85%] justify-center space-x-4 responsive-btns md:ml-24 md:justify-end">
                    @if ($ioData)
