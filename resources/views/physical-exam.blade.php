    ...
    <form action="{{ route('physical-exam.store') }}" method="POST" class="cdss-form"
        ...
        <fieldset @if (!session('selected_patient_id')) disabled @endif>
                    ...
                    <div class="w-[68%] rounded-[15px] overflow-hidden">
                        <table class="w-full border-separate border-spacing-0">
                            ...
                            <tr>
                                ...
                                <td class="bg-beige border-b-2 border-line-brown">
                                    <textarea name="extremities" class="notepad-lines cdss-input w-full h-[9G}ideo' border-none"
            ...
            <div class="w-[70%] mx-auto flex justify-end mt-5 mb-30 space-x-4">
                ...
                {{-- This button links to the new wizard.
                     It ONLY works if '$physicalExam' is set (meaning you are EDITING an existing exam).
                     This is because we need a '$physicalExam->id' to attach the diagnosis to. --}}

                @if (isset($physicalExam))
                    {{-- IF we are EDITING, link to the start of the wizard --}}
                    <a href="{{ route('nursing-diagnosis.start', $physicalExam->id) }}" class="button-default">
                        CDSS
                    </a>
                @else
                    {{-- IF we are CREATING, disable the button. User must save first. --}}
                    <button type="button" class="button-default opacity-50 cursor-not-allowed" disabled
                            title="You must SUBMIT the Physical Exam before adding a diagnosis.">
                        CDSS
                    </button>
                @endif
