...
@section('content')
    ...
    {{-- COPIED FROM physical-exam.blade.php (with <fieldset> and <center>) --}}
            <form action="{{ route('nursing-diagnosis.storeDiagnosis', $physicalExamId) }}" method="POST"
                class="h-full flex flex-col cdss-form" data-analyze-url="{{ route('nursing-diagnosis.analyze-field') }}"
                data-patient-id="{{ $patient->patient_id }}">
                @csrf

                {{-- We know a patient is selected, so we don't need the 'disabled' attribute --}}
                <fieldset>
                    <div class="w-[70%] mx-auto flex justify-center items-start gap-0 mt-6">
                        ...
                        <div class="w-[25%] rounded-[15px] overflow-hidden ml-4">
                            ...
                            <div class="alert-box my-0 py-4 px-3 flex justify-center items-center w-full rounded-b-lg"
                                data-alert-for="diagnosis" style="border-top: none;"> {{-- Remove double border --}} <span
                                    class="opacity-70 text-white font-semibold">No Recommendations</span> </div>
                        </div>
                    </div>
...
@endsection
        ...
        @push('scripts')
            {{-- We only need adpie-alert.js for this page --}}
