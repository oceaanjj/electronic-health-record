...
window.initializeDateDayLoader = function (selectUrl) {
    ...
    if (!selectUrl) {
        ...
        selectUrl = dropdownContainer ? dropdownContainer.dataset.selectUrl : null;
    }
    ...
    const handleDateDayChange = async () => {
        ...
        if (!patientId || !date || !dayNo) {
            console.warn('Date/Day Loader: Cannot load data. Missing Patient ID, Date, or Day No.');
            return;
        }
            ...
            if (newContent) {
                ...
                if (newCdssForm) {
                    if (newCdssForm.id === 'vitals-form' && typeof window.initializeVitalSignsAlerts === 'function') {
                        ...
                        newCdssForm.id === 'io-form' &&
                        typeof window.initializeIntakeOutputAlerts === 'function'
                    ) {
    ...
    const isADLForm = initialDateSelector && initialDayNoSelector && initialPatientIdHidden && formContainer;
