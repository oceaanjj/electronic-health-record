    ...
    >
        ...
        <fieldset>
            <div class="mx-auto mt-2 w-[70%]">
                {{-- RECOMMENDATION BANNERS - KEEPING ORIGINAL BACKEND LOGIC --}}
                @php
                    $alert = session('lab-values-alerts')['planning'] ?? null;
...
{{ old('planning', $diagnosis->planning ?? '') }}</textarea
            ...
            <div class="mx-auto mt-8 mb-12 flex w-[70%] items-center justify-between">
                <div class="flex flex-col items-start">
                    <a
                        href="{{ route('nursing-diagnosis.start', ['component' => $component, 'id' => $diagnosis->patient_id]) }}"
...
@push('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Auto-save draft (COMPONENT-SPECIFIC)
            let saveTimeout;
            textarea.addEventListener('input', function () {
                ...
                saveTimeout = setTimeout(function () {
                    ...
                    if (patientId && component) {
                        localStorage.setItem(`[FIELD]_draft_${component}_${patientId}`, textarea.value);
                    }
                }, 1500);
            });

            // Load draft (COMPONENT-SPECIFIC)
            const patientId = '{{ $patient->patient_id ?? '' }}';
            const component = '{{ $component }}';
            if (patientId && component) {
                const draft = localStorage.getItem(`[FIELD]_draft_${component}_${patientId}`);
